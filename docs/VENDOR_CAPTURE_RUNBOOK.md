...
5. Supply an independent random key of at least 32 characters only through
   `MACRO_LAB_AUTHORIZATION_KEY`. Never persist it with the receipt or permit.
6. Issue the signed access receipt before the registered access-ready deadline,
   then issue an action-specific permit. Follow
   [the authorization runbook](CAPTURE_AUTHORIZATION_RUNBOOK.md).
7. Confirm the host clock is synchronized before every release window. The local
...
macro-lab capture-te-snapshot \
  --authorization-permit data/private/authorization/event-pre.json \
...
macro-lab capture-te-stream-jsonl \
  --authorization-permit data/private/authorization/event-stream.json \
  --rights-attestation data/private/vendor_rights.json
```

The recorder validates the permit before consuming input, then validates
`topic=calendar`, provider identity, JSON integrity and rights. It stamps the
complete in-memory message with local UTC and monotonic receipt clocks. Empty lines
are ignored. A message containing the configured credential is rejected before
persistence.
